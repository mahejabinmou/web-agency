...
const stats = [
  { count: 8705, label: "Projects Completed", speed: 4000 },
  { count: 480, label: "Active Clients", speed: 3000 },
  { count: 626, label: "Cups of Coffee", speed: 3000 },
  { count: 9704, label: "Happy Clients", speed: 4000 },
];
import BgImage from "/Images/image-1.jpg";
  ...
  return (
    <section className="relative bg-[#1c1e22] text-white pt-0 pb-[290px] mb-[15%] lg:mb-16 overflow-hidden">
      <img
        src={BgImage}
        alt=""
        className="absolute w-full h-full object-cover grayscale opacity-30"
      />
      <div className="relative container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 pt-32">
          {stats.map((item, index) => (
            <div
              key={index}
              className="relative text-center before:absolute before:left-[-15px] before:right-[-15px] before:top-0 before:border-b before:border-[#ffae00] before:scale-x-0 hover:before:scale-x-100 before:transition-transform after:absolute after:right-[-15px] after:top-0 after:bottom-2 after:border-r after:border-white/15 last:after:hidden"
            >
              <div>
                <div className="text-6xl font-light font-sans">
                  {item.count}
                </div>
                <div className="uppercase tracking-wider text-sm mt-2">
                  {item.label}
                </div>
              </div>
            </div>
